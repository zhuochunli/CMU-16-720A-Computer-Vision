...
rects_red = np.load("../data/carseqrects.npy")     # the origin tangle in Q1.3
...
for i in range(1, seq.shape[2]):
    ...
    p = LucasKanade(frame_prev, cur_frame, rect, threshold, num_iters, p0)
    p0 = np.copy(p)
    delta_p = [rect[0] + p[0] - origin[0], rect[1] + p[1] - origin[1]]
    p_star = LucasKanade(frame_temp, cur_frame, origin, threshold, num_iters, delta_p)

    if np.linalg.norm(p_star - p0) <= template_threshold:
    ...
    if i in [1, 100, 200, 300, 400]:
        ...
        old_rect = rects_red[i, :]  # rectangle in carseqrects.npy
        new_rect = rects[i, :]
...
np.save('../data/carseqrects-wcrt.npy', rects)